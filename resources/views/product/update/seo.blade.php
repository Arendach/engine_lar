...
<br>
<form action="@uri('product/update_seo')" class="form-horizontal" data-type="ajax">
    <input type="hidden" name="id" value="{{ $product->id }}">

    <div class="tab-content">
        @foreach(['uk', 'ru'] as $item)
            <div class="tab-pane{{ $item == 'uk' ? ' active' : '' }}" id="seo_{{ $item }}">
                <div class="form-group">
                    <label class="col-md-4 control-label">Заголовок (Title)</label>
                    <div class="col-md-5">
                        <input class="form-control" name="meta_title_{{ $item }}" value="{{ $product->{"meta_title_$item"} }}">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-5">
                        <input class="form-control" name="meta_keywords_{{ $item }}" value="{{ $product->{"meta_keywords_$item"} }}">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-5">
                        <input class="form-control" name="meta_description_{{ $item }}" value="{{ $product->{"meta_description_$item"} }}">
                    </div>
                </div>
            </div>
        @endforeach
